#!/usr/bin/env python3
"""
inspect_overpass_structure.py

Fetches a small sample of building ways in Noord-Holland
(without any solar filter) and pretty-prints their JSON keys
so you can see exactly which tags are present.
"""

import json
import logging
import requests
import config  # must live in the same folder

# 1) simple console logging
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s %(levelname)-7s %(message)s"
)

# 2) build a valid Overpass query (no limit in the 'out' line)
query = f"""
[out:json][timeout:25];
area({config.DEFAULT_AREA_ID})->.searchArea;
way["building"](area.searchArea);
out center;
"""

logging.info("Querying Overpass for building elements in Noord-Holland")
resp = requests.post(config.OVERPASS_URL, data=query)
resp.raise_for_status()

elements = resp.json().get("elements", [])
logging.info("Total elements returned: %d", len(elements))

# 3) only look at the first 5 for inspection
for i, el in enumerate(elements[:5], start=1):
    logging.info("Element #%d: OSM id=%s, type=%s", i, el.get("id"), el.get("type"))
    print("  top-level keys:", list(el.keys()))
    tags = el.get("tags", {})
    if tags:
        print("  tags keys    :", list(tags.keys()))
    print("-" * 60)
